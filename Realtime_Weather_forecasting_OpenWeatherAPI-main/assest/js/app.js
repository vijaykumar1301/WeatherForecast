...
export const updateWeather = (lat, lon) => {
	...
	if (window.location.hash == '#/current-location') currentLocationBtn.setAttribute('disabled', '');
...
document.getElementById('more-info-btn').addEventListener('click', function () {
	// Retrieve the searched location from the current page
	const searchField = document.querySelector('[data-search-field]');
	console.log(searchField);
	const searchedLocation = searchField.value;

	// Open the Windy map page with the searched location as a query parameter
	window.open(`windy.html?location=${encodeURIComponent(searchedLocation)}`, '_blank');
});

document.getElementById('more-info-btn').addEventListener('click', function () {
	// Retrieve the search field element
	const searchField = document.querySelector('[data-search-field]');

	// Check if the search field exists
	if (searchField) {
		// Retrieve the value from the search field
		const searchedLocation = searchField.value.trim();

		// Check if the searched location is not empty
		if (searchedLocation) {
			// Update the iframe's src attribute with the searched location
			const iframe = document.getElementById('windy-iframe');
			iframe.src = `https://embed.windy.com/embed.html?type=map&location=${encodeURIComponent(
				searchedLocation,
			)}&metricRain=default&metricTemp=default&metricWind=default&zoom=5&overlay=wind&product=ecmwf&level=surface&detail=true`;
		} else {
			// Handle the case where the search field is empty
			console.log('Search field is empty. Please enter a location.');
			alert('Please enter a location to search.');
		}
	} else {
		// Handle the case where the search field is not found
		console.log('Search field not found.');
		alert('Search field not found on the page.');
	}
});
